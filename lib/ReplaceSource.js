...
class ReplaceSource extends Source {
	...
	streamChunks(options, onChunk, onSource, onName) {
		...
		// When the wrapped source exposes a persistent split-lines cache
		...
		// The check is duck-typed so any Source that grows the same API
		// can participate.
		const innerSource =
				...
				if (replacementEnd > pos) {
					...
					if (replacementEnd >= endPos) {
						...
						if (chunk.endsWith("\n")) {
							...
							/* istanbul ignore next: pre-existing chunk-skipping edge case (also untested on main) */
							generatedColumnOffset = -chunk.length;
							...
							for (let m = 0; m < matches.length; m++) {
								...
								if (m === matches.length - 1 && !contentLine.endsWith("\n")) {
									if (generatedColumnOffsetLine === line) {
										...
										/* istanbul ignore next: pre-existing multi-line replacement edge case (also untested on main) */
										generatedColumnOffset = contentLine.length;
										generatedColumnOffsetLine = line;
									}
								} else {
		...
		if (remainer.length > 0 && !remainer.includes("\n")) {
			...
			if (generatedColumnOffsetLine === line) {
				...
				/* istanbul ignore next: trailing-remainer cross-line case (also untested on main) */
				generatedColumnOffset = remainer.length;
				generatedColumnOffsetLine = line;
			}
		} else {
			...
			for (let m = 0; m < matches.length; m++) {
				...
				if (m === matches.length - 1 && !contentLine.endsWith("\n")) {
					if (generatedColumnOffsetLine === line) {
						...
						/* istanbul ignore next: trailing-remainer multi-line cross-line case (also untested on main) */
						generatedColumnOffset = contentLine.length;
						generatedColumnOffsetLine = line;
					}
				} else {
