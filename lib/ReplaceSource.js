...
class ReplaceSource extends Source {
	 ...
	 * @param {OnOriginalScope=} onOriginalScope called for each original scope (Scopes Proposal — forwarded as-is; positions are not remapped through replacements)
	 * @param {OnGeneratedRange=} _onGeneratedRange called for each generated range (Scopes Proposal — dropped; positions cannot be safely remapped through replacements)
	...
	streamChunks(
		...
		const { generatedLine, generatedColumn } = streamChunks(
			...
			// Forward original-scope data unchanged (positions refer to
			// original sources, not to generated columns, so they're stable
			// across replacements). Generated ranges are dropped because their
			// positions refer to the generated code whose columns the
			// replacements shift.
			onOriginalScope,
			undefined,
		);
