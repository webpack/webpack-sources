...
class ReplaceSource extends Source {
	...
	streamChunks(options, onChunk, onSource, onName) {
		...
		// When the wrapped source can hand us a persistent split-lines cache
		// (e.g. `OriginalSource.originalLines()`), reuse it instead of
		// re-running `splitIntoLines` on every `streamChunks` call. The
		// in-call cache stored in `sourceContents` already memoizes per
		// streamChunks invocation; this extends that memoization across
		// invocations of the same `ReplaceSource` (uncached map() loops,
		// sourceAndMap() called after map(), and similar repeated reads).
		// Hoist the duck-type check to a boolean so the closure doesn't
		// allocate a bound function per streamChunks call — we call
		// `innerSource.originalLines()` directly on demand instead.
		const innerSource =
			/** @type {Source & { originalLines?: () => string[] }} */
			(this._source);
		const innerHasOriginalLines =
			typeof innerSource.originalLines === "function";
		/**
			...
			if (typeof raw === "string") {
				...
				lines =
					sourceIndex === 0 && innerHasOriginalLines
						...
						: splitIntoLines(raw);
				sourceContents[sourceIndex] = lines;
			} else {
				lines = raw;
			}
			const contentLine = line <= lines.length ? lines[line - 1] : null;
		...
		const { generatedLine, generatedColumn } = streamChunks(
			...
			(
				...
				if (nextReplacement < endPos) {
					do {
						...
						// Fast path: most replacements (insert tokens, rename
						// identifiers) carry single-line content. Skip
						// splitIntoLines and the per-match loop entirely.
						// Empty content is treated as a no-op chunk because
						// splitIntoLines("") returns an empty array; emitting a
						// zero-length chunk would change the mappings output.
						if (content.length > 0 && !content.includes("\n")) {
							...
							// Nothing to emit; fall through to next replacement.
						} else {
			...
			(sourceIndex, source, sourceContent, info) => {
				while (sourceContents.length < sourceIndex) {
					sourceContents.push(undefined);
				}
				sourceContents[sourceIndex] = sourceContent;
				onSource(sourceIndex, source, sourceContent, info);
			},
			(nameIndex, name) => {
				let globalIndex = nameMapping.get(name);
				if (globalIndex === undefined) {
