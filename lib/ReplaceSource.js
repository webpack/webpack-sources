...
class ReplaceSource extends Source {
	...
	replace(start, end, newValue, oldValue) {
		...
		this.replacements.push([start, end, newValue, this.replacements.length, oldValue]);
	}

	insert(pos, newValue) {
		if(typeof newValue !== "string")
			throw new Error("insertion must be a string, but is a " + typeof newValue + ": " + newValue);
		this.replacements.push([pos, pos - 1, newValue, this.replacements.length, undefined]);
	}
