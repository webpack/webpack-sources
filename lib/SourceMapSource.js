...
class SourceMapSource extends Source {
	...
	clearCache(options, visited) {
		...
		if (options !== undefined && options.size === true) {
			this._cachedSize = undefined;
		}
		// For every dual-cached pair, drop the string form when the
		// buffer counterpart is also held. The buffer is more compact
		// in V8 (1 byte/char vs 2) and the string is cheap to rehydrate.
		// Parsed object forms of source maps are heavier but only dropped
		// when `parsedMap: true` is set, because re-parsing JSON is much
		// more expensive than `toString` from a buffer.
		if (
			clearSource &&
