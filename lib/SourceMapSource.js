...
class SourceMapSource extends Source {
	...
	map(options) {
		...
		return this._withOuterExtras(getMap(this, options));
	}
	...
	sourceAndMap(options) {
		...
		const sourceAndMap = getSourceAndMap(this, options);
		sourceAndMap.map = this._withOuterExtras(sourceAndMap.map);
		return sourceAndMap;
	}

	/**
	 * Re-attach `debugId` and `sourceRoot` from the outer source map after
	 * a getMap()/getSourceAndMap() round-trip. The stream-based pipeline
	 * only carries `sources`, `sourcesContent`, `names`, and `mappings`;
	 * whole-map fields would otherwise be silently dropped whenever
	 * `SourceMapSource` is composed with an inner source map. `ignoreList`
	 * is already preserved per-source via the streamChunks pipeline, so
	 * it isn't copied here (doing so would discard inner-source ignored
	 * indices the pipeline already correctly produced).
	 * @private
	 * @param {RawSourceMap | null} map result of getMap/getSourceAndMap
	 * @returns {RawSourceMap | null} map with outer extras merged in
	 */
	_withOuterExtras(map) {
		if (map === null) return null;
		const outer = this._sourceMapObject();
		if (outer === null) return map;
		if (outer.debugId !== undefined && map.debugId === undefined) {
			map.debugId = outer.debugId;
		}
		if (outer.sourceRoot !== undefined && map.sourceRoot === undefined) {
			map.sourceRoot = outer.sourceRoot;
		}
		return map;
	}
