...
const streamChunksOfCombinedSourceMap = (
	...
	/** @type {undefined | import("./streamChunks").SourceInfo} */
	let innerSourceNameInfo;
	/** @type {number[]} */
	const innerSourceIndexMapping = [];
	/** @type {[string | null, string | undefined, (undefined | import("./streamChunks").SourceInfo)][]} */
	...
	return streamChunksOfSourceMap(
					...
					if (innerSourceIndex >= 0) {
						...
						let sourceIndex =
							innerSourceIndex < innerSourceIndexMapping.length
								...
								: -2;
						if (sourceIndex === -2) {
							const [source, sourceContent, info] =
							...
							if (globalIndex === undefined) {
								...
								onSource(globalIndex, source, sourceContent, info);
							}
		...
		(i, source, sourceContent, info) => {
			if (source === innerSourceName) {
				outerSourceIndex = i;
				// Remember the outer source's metadata for the
				// "outer points at inner source but no inner mapping" case
				// further down — that emission has to attribute the inner
				// source the same ignored/etc. info the outer map carried.
