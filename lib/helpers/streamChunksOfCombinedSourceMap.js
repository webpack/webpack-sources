	...
	streamChunksOfSourceMap(
		...
		(
			...
			if (sourceIndex === innerSourceIndex) {
				...
				const idx = findInnerMapping(originalLine, originalColumnn);
				if (idx !== -1) {
					...
					if (innerSourceIndex >= 0) {
						...
						// where we are allowed to adject the original column
						const innerChunk = chunks[idx];
						const innerGeneratedColumn = mappingsData[mi];
						const locationInChunk = originalColumnn - innerGeneratedColumn;
