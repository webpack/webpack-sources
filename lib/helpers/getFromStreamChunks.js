...
module.exports.getMap = (source, options) => {
	...
	if (mappings.length === 0 && !hasScopesData) return null;
	...
	if (hasOriginalScopes) map.originalScopes = originalScopes;
	if (generatedRanges.length > 0) map.generatedRanges = generatedRanges;
	return map;
};
