...
module.exports.getMap = (source, options) => {
	...
	source.streamChunks(
		...
		// Inlined "set at index" padding: `push(null)` keeps the array in
		// V8's PACKED_ELEMENTS kind (writing `arr.length = i + 1` would
		// force HOLEY_ELEMENTS even after filling the gap, which costs
		// memory in benchmarks like source-map-source combined-inner).
		// Skipping the extracted helper avoids the per-source function-call
		// overhead the streaming pipeline would otherwise pay.
