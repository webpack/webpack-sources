...
const streamChunksOfSourceMapFull = (
	...
	const { sources, sourcesContent, names, mappings, ignoreList } = sourceMap;
	// Set membership is the only operation we run against ignoreList per
	// source; building a Set once amortizes the work across the loop and
	// avoids quadratic Array.prototype.includes scans for maps with large
	// ignoreLists.
	const ignoreSet =
		ignoreList && ignoreList.length > 0 ? new Set(ignoreList) : undefined;
	for (let i = 0; i < sources.length; i++) {
		onSource(
			i,
			getSource(sourceMap, i),
			(sourcesContent && sourcesContent[i]) || undefined,
			ignoreSet !== undefined && ignoreSet.has(i) ? IGNORED_INFO : undefined,
		);
	}
	if (names) {
...
const streamChunksOfSourceMapLinesFull = (
	...
	const { sources, sourcesContent, mappings, ignoreList } = sourceMap;
	const ignoreSet =
		ignoreList && ignoreList.length > 0 ? new Set(ignoreList) : undefined;
	for (let i = 0; i < sources.length; i++) {
		onSource(
