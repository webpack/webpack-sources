...
const createMappingsIterator = require("./createMappingsIterator");
...
const streamChunksOfSourceMapFull = (
	...
	const it = createMappingsIterator(mappings);
	while (it.next()) {
		const {
			generatedLine,
			generatedColumn,
	...
	if (finalLine > currentGeneratedLine && currentGeneratedColumn > 0) {
	...
	while (finalLine > currentGeneratedLine) {
		onChunk(
			lines[currentGeneratedLine - 1],
			currentGeneratedLine,
			0,
