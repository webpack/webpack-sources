...
const streamChunksOfSourceMapFull = (
	...
	const { originalScopes } = sourceMap;
	const { generatedRanges } = sourceMap;
	...
	if (onOriginalScope) readAllOriginalScopes(originalScopes, onOriginalScope);
	if (onGeneratedRange) readGeneratedRanges(generatedRanges, onGeneratedRange);
	return {
		generatedLine: finalLine,
