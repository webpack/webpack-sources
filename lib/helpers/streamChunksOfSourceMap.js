...
const readAllOriginalScopes = require("./readAllOriginalScopes");
const readGeneratedRanges = require("./readGeneratedRanges");
...
const streamChunksOfSourceMapFull = (
	...
	readAllOriginalScopes(originalScopes, onOriginalScope);
	readGeneratedRanges(generatedRanges, onGeneratedRange);
	return {
		generatedLine: finalLine,
 ...
 * @param {function(string, number, number, number, number, number, number): void} onChunk called for each chunk of code
 * @param {function(number, string, string)} onSource called for each source
 * @param {function(number, string)} _onName called for each name
 * @param {OnOriginalScope} onOriginalScope called for each original scope
 * @param {OnGeneratedRange} onGeneratedRange called for each generated range
 * @returns {{ generatedLine: number, generatedColumn: number }} final generated position
 */
const streamChunksOfSourceMapLinesFull = (
	...
	const { sources, sourcesContent, mappings } =
		sourceMap;
	for (let i = 0; i < sources.length; i++) {
		onSource(
			i,
