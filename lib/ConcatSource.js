		...
		for (let ci = 0; ci < childCount; ci++) {
			...
			const { generatedLine, generatedColumn, source } = streamChunks(
				...
				(i, source, sourceContent, info) => {
					...
					} else if (info !== undefined && info.ignored) {
						// A later child may mark the same source as ignored even
						// if the first child didn't. Re-emit (the consumer's
						// getMap collector treats this as a union; downstream
						// listeners are no-ops for re-emits).
						onSource(globalIndex, source, sourceContent, info);
					}
