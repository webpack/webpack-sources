...
class ConcatSource extends Source {
	...
	streamChunks(options, onChunk, onSource, onName) {
		...
		for (const item of this._children) {
			...
			let lastLine = currentLineOffset + 1;
			let lastColumn = currentColumnOffset;
			let lastChunk = "";
			streamChunks(
				item,
				options,
			...
			if (
				lastChunk.length > 0 &&
				lastChunk.charCodeAt(lastChunk.length - 1) === NEW_LINE_CHAR_CODE
			) {
				currentLineOffset = lastLine;
				currentColumnOffset = 0;
			} else {
				currentLineOffset = lastLine - 1;
				currentColumnOffset = lastColumn + lastChunk.length;
			}
		}
	}
