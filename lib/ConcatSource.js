...
class ConcatSource extends Source {
	...
	streamChunks(options, onChunk, onSource, onName) {
		...
		// Per-source-extras side-channel (currently `ignoreList`). If the
		// caller didn't supply one we don't allocate a wrapped options
		// object below — most map() calls don't carry ignoreList input.
		...
		for (let ci = 0; ci < childCount; ci++) {
			...
			const childOptions =
				outerOnSourceInfo === undefined
					? options
					: {
							...options,
							// Remap child→global source index before forwarding
							// per-source extras to the caller. Allocated only
							// when info propagation is actually in play.
							onSourceInfo:
								/**
								 * @param {number} childIdx child idx
								 * @param {import("./helpers/streamChunks").SourceInfo} info info
								 * @returns {void}
								 */
								(childIdx, info) => {
									if (childIdx < 0 || childIdx >= sourceIndexMapping.length) {
										return;
									}
									const globalIdx = sourceIndexMapping[childIdx];
									if (globalIdx !== undefined && globalIdx >= 0) {
										outerOnSourceInfo(globalIdx, info);
									}
								},
						};
