 ...
 * handed to a caller carries no big string (they deserialize far slower).
 * Hashing a string reads it as utf8, so the encoded update hashes the same.
 * @param {(string | Buffer)[]=} update recorded hash update
 * @returns {(string | Buffer)[]=} update holding buffers in place of strings
 */
const bufferHashUpdate = (update) => {
	if (update === undefined) return undefined;
	let index = 0;
	while (index < update.length && typeof update[index] !== "string") index++;
	if (index === update.length) return update;
	const buffered = [...update];
	for (; index < buffered.length; index++) {
		const item = buffered[index];
		if (typeof item === "string") buffered[index] = Buffer.from(item, "utf8");
	}
	return buffered;
};
