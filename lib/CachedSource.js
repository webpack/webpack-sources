...
class CachedSource extends Source {
	...
	clearCache(options, visited) {
		...
		const clearHash = options !== undefined && options.hash === true;
		const clearSize = options !== undefined && options.size === true;
		...
		if (clearMaps) {
			// `_cachedMaps` is keyed by stable cache keys (see `getCacheKey`)
			// so reusing the Map avoids per-call allocation churn that
			// builds calling `clearCache` thousands of times would feel.
		...
		if (recursive && typeof this._source !== "function") {
			let v = visited;
			if (v === undefined) {
				v = new WeakSet();
			}
