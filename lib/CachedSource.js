 ...
 * @typedef {{ map?: null | RawSourceMap, bufferedMap?: null | BufferedMap, scopeBindings?: (ScopeBindings | undefined)[] }} BufferEntry
 */
...
class CachedSource extends Source {
	...
	getCachedData() {
		...
		for (const pair of this._cachedMaps) {
			...
			if (cacheEntry.scopeBindings !== undefined) {
				bufferEntry.scopeBindings = cacheEntry.scopeBindings;
			}
	...
	streamChunks(options, onChunk, onSource, onName) {
		...
		if (
			...
			(!scopes || cacheEntry.scopeBindings !== undefined) &&
			(this._cachedBuffer !== undefined || this._cachedSource !== undefined)
		) {
			const { source, map } = this.sourceAndMap(options);
			if (map) {
				const { scopeBindings } = cacheEntry;
				return streamChunksOfSourceMap(
					/** @type {string} */
					(source),
					map,
					onChunk,
					scopes && scopeBindings !== undefined
						? (sourceIndex, source, sourceContent) =>
								onSource(
									sourceIndex,
									source,
									sourceContent,
									scopeBindings[sourceIndex],
								)
						: onSource,
					onName,
		...
		/** @type {(ScopeBindings | undefined)[] | undefined} */
		let scopeBindings;
		const sourceAndMap = streamAndGetSourceAndMap(
			...
			scopes
				? (sourceIndex, source, sourceContent, bindings) => {
						const recorded = scopeBindings || (scopeBindings = []);
						recorded[sourceIndex] = bindings;
		...
		this._cachedMaps.set(key, {
			...
			bufferedMap: undefined,
			scopeBindings: scopes ? scopeBindings || [] : undefined,
		});
