...
function isMultiComment(code) {
	...
	var mCommentStartIndex = code.indexOf('/*');
	if(mCommentStartIndex === 0) {
...
function isSingleComment(code, identifierIndex) {
	var sCommentIndex = code.indexOf('//');
	if(sCommentIndex !== -1 && sCommentIndex < identifierIndex) {
		return true;
	}
...
function splitCodeByIdentifiers(code) {
	var result = [];
	var re = IDENTIFIER_REGEX;
	var index = 0;

	for(var identifier = re.exec(code); identifier !== null; identifier = re.exec(code)) {
		if(isSingleComment(code, identifier.index)) {
			re.lastIndex = 0;
			break;
		}
		var word = identifier[0];
		if(RESERVED_WORDS_REGEX.test(word)) {
			continue;
		}

		// text before identifier
		if(index < identifier.index) {
			result.push({
				text: code.substring(index, identifier.index),
				pos: index,
			});
		}
		// identifier
	...
	if(index < code.length) {
		var lastWord = index === 0 ? code.substring(index) : code.substring(index).replace('\n', '');
		result.push({
			text: lastWord,
			pos: index,
		});;
	...
	node(options) {
		...
		var node = new SourceNode(null, null, null,
			lines.map(function(line, idx) {
				...
				if(options.columns === false || isMultiComment(content)) {
					return new SourceNode(idx + 1, 0, name, content);
				}
				return new SourceNode(null, null, null,
					splitCodeByIdentifiers(content).map(function(item) {
						if(/^\s*$/.test(item.text)) {
							return item.text;
						}
						return new SourceNode(idx + 1, item.pos, name, item.text);
					})
				);
			})
		);
