...
class OriginalSource extends Source {
	...
	/**
	 * Override Source.prototype.size to avoid allocating a Buffer just to
	 * read the byte length when one isn't already cached.
	 * @returns {number} byte length
	 */
	size() {
		if (this._valueAsBuffer !== undefined) return this._valueAsBuffer.length;
		return Buffer.byteLength(/** @type {string} */ (this._value), "utf8");
	}
