...
class OriginalSource extends Source {
	...
	constructor(value, name) {
		 ...
		 */
		this._name = name;
		// `this._lines` is assigned lazily by `originalLines()` only when a
		// caller (typically `ReplaceSource.checkOriginalContent`) actually
		// needs the split-lines cache. Most `OriginalSource` instances are
		// constructed, hashed, and serialized without ever asking for the
		// cache, so initializing the slot here would cost one hidden-class
		// transition on every construction with no payoff.
	}
	 ...
	 * @returns {string[]} cached source content lines
	 */
	originalLines() {
		if (this._lines !== undefined) return this._lines;
		const value =
			this._value !== undefined
				? this._value
				: /** @type {Buffer} */ (this._valueAsBuffer).toString("utf8");
		if (this._value === undefined && isDualStringBufferCachingEnabled()) {
			this._value = value;
		}
		return (this._lines = splitIntoLines(value));
	}
	...
	clearCache(options, visited) {
		...
		// The split-lines cache is derived from the source string. Drop it
		// alongside source-form clearing so the heap doesn't hold both the
		// string array (held by `_lines`) and the buffer simultaneously.
		// Skip the assignment when the slot was never created — touching it
		// here would force a hidden-class transition on instances that never
		// asked for the cache.
		if (this._lines !== undefined) this._lines = undefined;
	}
